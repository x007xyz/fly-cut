...
// 获取当前scale下的单元格像素
export function getGridPixel(scale: number, frameCount: number) {
...
// 根据缩放比调整 step
function getStep(scale: number, frameStep: number): number {
  return scale > 60 ? frameStep : 10
}
// 转换时间格式
export function getLongText(count: number, scale: number) {
...
function getShortText(count: number, step: number, scale: number) {
...
// 获取选中点的帧坐标
export function getSelectFrame(offsetX: number, scale: number, frameStep: number) {
 ...
 * 时间轴画线
 */
export function drawTimeLine(context: CanvasRenderingContext2D, userConfigs: UserConfig, canvasConfigs: CanvasConfig) {
