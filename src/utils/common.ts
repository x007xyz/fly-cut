...
export function calcTrackItemAttr(trackItem: Record<string, any>, canvasSize: { width: number, height: number }) {
...
export function file2ArrayBuffer(file: File): Promise<ArrayBuffer> {
...
export async function file2Unit8Stream(file: File): Promise<ReadableStream<Uint8Array>> {
 ...
 * @param frame
 */
export function getCurSubtitle(asr: { beginTime: number, endTime: number, text: string }[], frame: number) {
  // 将frame转换为当前时间
