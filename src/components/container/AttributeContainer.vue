...
<template>
  <div class="select-none relative pl-2" :style="attrWidth">
    <SplitLine
      v-model:new-width="pageStore.attrWidth" class="top-0 left-0 bottom-0" direction="vertical" :limit-size="limitSize"
      disabled
    />
    ...
    <div class="absolute top-0 left-3 right-2 bottom-0 overflow-hidden">
      <AttrContainer :attr-data="selectTrackOptionsConfig" :track-id="trackStore.selectResource?.id" />
    </div>
  </div>
</template>
